...
function checkPasswordExists($db, $password, $id)
{# TODO
    $sql = "SELECT COUNT(*) FROM personen WHERE password = :password AND id = :id";
    $daten = [':password' => password_hash($password, PASSWORD_DEFAULT), ':id' => $_SESSION['id']];
    $stmt = $db->prepare($sql);
    $stmt->execute($daten);
